...
const Notepad = _ => {
    const [notas, setNotas] = useState([{id: Math.random(), title: "01", text: "02"}, {id: Math.random(), title: "03", text: "04"}])
    ...
    const btnPress = (e, id) => {
        ...
        if(e.target.value === "Close"){
            ...
            Note.style.maxWidth = "calc(100% - 3em)"
            Note.style.maxHeight = "calc(100% - 3em)"
        }else{
            InputTitle.style.fontSize = "0.5em"
            TextArea.style.fontSize = "0.5em"
            Note.style.maxWidth = "calc(25% - 3em)"
            Note.style.maxHeight = "calc(10% - 3em)"
        }
    }
    useEffect(() => {
        console.log(notas)
